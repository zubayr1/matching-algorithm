...
namespace FlexchainFunction
{
    public class MaPW
    {
        ...
        public MaPW(List<FlexibilityRequest> flexRequest, List<FlexibilityOffer> flexOffer)
        {
        ...
        public Dictionary<string, int> Matching(List<string> userlist, FlexibilityRequest flexibilityRequest, int TOTALFLEXREQUESTED)
        {
            ...
            {
                var PEFACTOR = new Dictionary<string, int>();

                PEFACTOR = flexibilityRequest.Loc;

                PEFACTOR = PEFACTOR.OrderByDescending(x => x.Value).ToDictionary(x => x.Key, x => x.Value);

                foreach(var i in PEFACTOR.Keys)
                {
                    if(TOTALFLEXREQUESTED> POTENTIALOFFER[i])
                    {
        ...
        public Flex_MatchingAlgo MaPWCalculation()
        {
            var accepted_offers = new Dictionary<string, int>();
